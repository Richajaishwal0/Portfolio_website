...
const SideNavigation: React.FC<SideNavigationProps> = ({ darkMode, language }) => {
  ...
  return (
    <>
      ...
      <nav className={`fixed left-0 top-0 h-full w-20 z-50 hidden lg:flex flex-col items-center justify-center transition-all duration-700 ${
        darkMode 
        ...
        <div className="flex flex-col space-y-4">
          {navItems.map((item) => (
            <button
              ...
              title={item.label}
            >
              <span className={`text-2xl transition-all duration-300 group-hover:scale-125 group-hover:drop-shadow-[0_0_8px_rgba(236,72,153,0.7)] group-hover:text-pink-400 dark:group-hover:text-cyan-300 ${
                activeSection === item.id 
              ...
              {/* Tooltip */}
              <div className={`absolute left-full ml-4 top-1/2 transform -translate-y-1/2 px-3 py-2 rounded-lg text-sm font-medium opacity-0 group-hover:opacity-100 transition-all duration-300 whitespace-nowrap pointer-events-none ${
                darkMode 
                ...
                <div className={`absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-1 w-2 h-2 rotate-45 ${
                  darkMode ? 'bg-purple-800/90 border-l border-b border-purple-600/50' : 'bg-orange-100/90 border-l border-b border-orange-300/50'
                }`} />
              </div>
            </button>
          ))}
      ...
      <nav className={`fixed bottom-0 left-0 right-0 z-50 lg:hidden transition-all duration-700 ${
        darkMode 
        ...
        <div className="flex justify-around items-center py-2">
          {navItems.map((item) => (
            ...
            >
              <span className={`text-xl transition-all duration-300 group-hover:scale-125 group-hover:drop-shadow-[0_0_8px_rgba(236,72,153,0.7)] group-hover:text-pink-400 dark:group-hover:text-cyan-300 ${
