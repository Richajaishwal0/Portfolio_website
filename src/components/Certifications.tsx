...
const Certifications: React.FC<CertificationsProps> = ({ darkMode, language }) => {
  const content = {
    en: {
  ...
  const certifications = [
    {
      ...
      year: '2024',
      icon: '🌐',
      description: 'Comprehensive understanding of IT infrastructure, networking protocols, server management, and cloud technologies.',
      skills: ['Networking', 'Server Management', 'Cloud Computing', 'IT Infrastructure'],
    ...
    },
    {
      ...
      year: '2024',
      icon: '💻',
      description: 'Advanced software development methodologies, best practices, and industry insights for modern development.',
      skills: ['Software Development', 'Best Practices', 'Methodologies', 'Industry Insights'],
    ...
    },
    {
      ...
      year: '2024',
      icon: '🎤',
      description: 'Professional communication skills, presentation techniques, and public speaking confidence building.',
      skills: ['Public Speaking', 'Communication', 'Presentation', 'Confidence Building'],
    ...
    },
    {
      ...
      year: '2024',
      icon: '🎨',
      description: 'Modern front-end development techniques, responsive design, and user interface best practices.',
    ...
    },
    {
      ...
      year: '2024',
      icon: '🤖',
      description: 'Comprehensive AI fundamentals, machine learning algorithms, and practical AI implementation.',
      skills: ['AI Fundamentals', 'Machine Learning', 'Neural Networks', 'AI Implementation'],
    ...
    },
    {
      ...
      year: '2024',
      icon: '🐍',
      description: 'Python programming fundamentals, data structures, algorithms, and problem-solving techniques.',
    ...
    },
    {
      ...
      year: '2024',
      icon: '🏆',
      description: 'Advanced problem-solving skills and competitive coding experience across platforms.',
      skills: ['Problem Solving', 'Competitive Coding', 'Data Structures', 'Algorithms'],
      color: darkMode ? 'from-blue-500 to-indigo-500' : 'from-blue-500 to-indigo-500'
    }
  ];
  ...
  const certFileMap: Record<string, { type: 'image' | 'pdf', src: string }> = {
    ...
    'IT Backbone: Networking, Servers & Cloud': { type: 'image', src: '/Networking.jpg' },
      ...
      <div className="container mx-auto px-4 sm:px-6 md:px-8">
            ...
            return (
              <CardWrapper key={index} {...cardProps}>
                <div className="flex items-start space-x-6 mb-6">
                  <div className={`p-4 rounded-2xl bg-gradient-to-r ${cert.color} group-hover:scale-110 group-hover:rotate-12 transition-all duration-500 shadow-lg`}>
                    <span className="text-3xl">{cert.icon}</span>
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className={`font-bold text-lg transition-all duration-300 group-hover:scale-105 ${
                        darkMode ? 'text-white' : 'text-gray-900'
                      }`}>
                        {cert.title}
                      </h3>
                      <div className="flex items-center space-x-2">
                        <CheckCircle size={18} className={darkMode ? 'text-green-400' : 'text-green-600'} />
                      </div>
                    </div>
                ...
                <div className="space-y-4">
                  <h4 className={`text-sm font-semibold transition-all duration-300 group-hover:scale-105 ${
        ...
        <div className="mt-12 md:mt-16 lg:mt-20 grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 lg:gap-10">
          {[
            ...
            { number: '2024', label: 'Latest Year', icon: '📅' },
        ...
        {/* Learning Philosophy */}
        <div className={`mt-16 p-10 rounded-2xl text-center transition-all duration-500 hover:scale-105 cursor-pointer ${
          darkMode 
          ...
          <h3 className={`text-3xl font-bold mb-6 transition-all duration-300 hover:scale-105 ${
            darkMode ? 'text-white' : 'text-gray-900'
          }`}>
            🎯 Continuous Learning Philosophy
          </h3>
          <p className={`text-xl max-w-4xl mx-auto transition-all duration-300 hover:scale-105 ${
            darkMode ? 'text-slate-300' : 'text-gray-700'
          }`}>
            "I believe in continuous learning and staying updated with the latest technologies. 
            Each certification represents not just knowledge gained, but a commitment to excellence 
            and professional growth in the ever-evolving field of technology."
          </p>
        </div>

        {/* Modal for certifications */}
        {openModal && openModal !== 'Competitive Programming' && certFileMap[openModal] && (
          <>
            <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm" onClick={() => setOpenModal(null)} />
            <div className="fixed inset-0 z-50 flex items-center justify-center">
              <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-4 w-[340px] sm:w-[400px] flex flex-col items-center border border-gray-200 dark:border-slate-700">
                <button
                  ...
                  onClick={e => { e.stopPropagation(); setOpenModal(null); }}
                  aria-label="Close"
                >
                  ×
                </button>
                <h4 className="text-lg font-bold mb-2 text-center text-gray-900 dark:text-white">{openModal} Certificate</h4>
                {certFileMap[openModal].type === 'image' ? (
                  <img src={certFileMap[openModal].src} alt={openModal + ' Certificate'} className="rounded-lg shadow-md max-h-64 w-auto" />
                ) : (
                  <iframe src={certFileMap[openModal].src} title={openModal + ' Certificate'} className="rounded-lg shadow-md w-full h-64" />
                )}
              </div>
            </div>
          </>
          ...
          <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center">
              <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-4 w-[340px] sm:w-[400px] flex flex-col items-center border border-gray-200 dark:border-slate-700">
                ...
                <h4 className="text-lg font-bold mb-2 text-center text-gray-900 dark:text-white">Competitive Programming Certificate</h4>
                <div className="flex items-center justify-center w-full mb-2">
                  <button
                    ...
                    onClick={e => { e.stopPropagation(); setCpIndex((cpIndex - 1 + cpFiles.length) % cpFiles.length); }}
                    disabled={cpFiles.length <= 1}
                  >
                    ‹
                  </button>
                  <span className="mx-2 text-sm text-gray-700 dark:text-gray-200">{cpFiles[cpIndex].label}</span>
                  <button
                    className="px-2 py-1 text-lg font-bold text-gray-500 hover:text-gray-900 dark:hover:text-white disabled:opacity-30"
                    onClick={e => { e.stopPropagation(); setCpIndex((cpIndex + 1) % cpFiles.length); }}
                    disabled={cpFiles.length <= 1}
                  >
                    ›
                  </button>
                ...
                {cpFiles[cpIndex].type === 'image' ? (
                  <img src={cpFiles[cpIndex].src} alt={cpFiles[cpIndex].label + ' Certificate'} className="rounded-lg shadow-md max-h-64 w-auto" />
                ) : (
                  <iframe src={cpFiles[cpIndex].src} title={cpFiles[cpIndex].label + ' Certificate'} className="rounded-lg shadow-md w-full h-64" />
                )}
              </div>
            </div>
          </>
