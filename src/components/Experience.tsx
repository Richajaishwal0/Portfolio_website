  ...
  const experiences = [
    {
      ...
      type: "Internship",
      description:
        "Worked on real-world AI projects and gained hands-on experience with artificial intelligence applications.",
      highlights: [
    ...
    },
    {
      ...
      type: "Achievement",
      description:
        "Participated in 7+ coding competitions on Unstop platform and received appreciation from senior engineers.",
      highlights: [
    ...
    },
    {
      ...
      type: "Certification",
      description:
        "Achieved 5-star rating in Python, Java & C++ on HackerRank platform, demonstrating strong programming skills.",
      highlights: [
    ...
    },
    {
      ...
      type: "Certification",
      description:
        "Certified as Postman Student Expert, specializing in API testing and development workflows.",
      highlights: [
  ...
  const openSourceContributions = [
    {
      ...
      description:
        "Contributed PRs, bug fixes, and enhancements across multiple projects",
      icon: "🌟",
    ...
    },
    {
      ...
      description:
        "3+ accepted PRs, documentation fixes, and GitHub community participation",
      icon: "🎃",
  ...
  return (
    ...
    >
      <div>
        <div className="container mx-auto px-4 sm:px-6 md:px-8">
            ...
            {experiences.map((exp, index) => {
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    if (exp.title === "Infosys Springboard Internship (AI)")
                      setOpenExpModal(exp.title);
                    if (exp.title === "Competitive Programming Excellence") {
                      setOpenExpModal(exp.title);
                      setCpExpIndex(0);
                    }
                    if (exp.title === "HackerRank 5-Star Achievement")
                      setOpenExpModal(exp.title);
                    if (exp.title === "Postman Student Expert")
                      setOpenExpModal(exp.title);
                  }}
                >
                  <div className="flex items-start space-x-6 mb-6">
                    <div
                      className={`p-4 rounded-2xl bg-gradient-to-r ${exp.color} group-hover:scale-110 group-hover:rotate-12 transition-all duration-500 shadow-lg`}
                    >
                    ...
                    <div className="flex-1">
                      ...
                      <p
                        className={`text-sm mb-4 transition-all duration-300 group-hover:scale-105 ${
          ...
          <div className="mb-12 md:mb-16 lg:mb-20">
            <h3
              className={`text-3xl font-bold text-center mb-12 transition-all duration-500 hover:scale-105 ${
                darkMode ? "text-white" : "text-gray-900"
              }`}
            >
              🌟 Open-Source Contributions
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {openSourceContributions.map((contrib, index) => (
                <div
                  ...
                  onClick={() => {
                    if (contrib.title === "GSSoC Extended 2024")
                      setShowGssocModal(true);
                    if (contrib.title === "Hacktoberfest 2024")
                      setShowHacktoberModal(true);
                  }}
                >
                  ...
                  <p
                    className={`text-lg mb-4 transition-all duration-300 group-hover:scale-105 ${
                      darkMode ? "text-slate-300" : "text-gray-700"
                    }`}
                  >
                    {contrib.description}
                  ...
                  <div
                    className={`text-sm px-4 py-2 rounded-full inline-block font-medium transition-all duration-300 group-hover:scale-105 ${
                      darkMode
      ...
      {openExpModal &&
        ...
        expFileMap[openExpModal] && (
          <>
            <div
              className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm"
              onClick={() => setOpenExpModal(null)}
            />
            <div className="fixed inset-0 z-50 flex items-center justify-center">
              <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-4 w-[340px] sm:w-[400px] flex flex-col items-center border border-gray-200 dark:border-slate-700">
                ...
                <h4 className="text-lg font-bold mb-2 text-center text-gray-900 dark:text-white">
                  {openExpModal} Certificate
                </h4>
                {expFileMap[openExpModal].type === "image" ? (
                  <img
                    src={expFileMap[openExpModal].src}
                    alt={openExpModal + " Certificate"}
                    className="rounded-lg shadow-md max-h-64 w-auto"
                  />
                ) : (
                  <iframe
                    src={expFileMap[openExpModal].src}
                    title={openExpModal + " Certificate"}
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-4 w-[340px] sm:w-[400px] flex flex-col items-center border border-gray-200 dark:border-slate-700">
              ...
              <h4 className="text-lg font-bold mb-2 text-center text-gray-900 dark:text-white">
                Competitive Programming Certificates
              </h4>
              <div className="flex items-center justify-center w-full mb-2">
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    setCpExpIndex(
                      (cpExpIndex - 1 + cpExpFiles.length) % cpExpFiles.length
                    );
                  }}
                ...
                <span className="mx-2 text-sm text-gray-700 dark:text-gray-200">
                  {cpExpFiles[cpExpIndex].label}
                </span>
                <button
                  className="px-2 py-1 text-lg font-bold text-gray-500 hover:text-gray-900 dark:hover:text-white disabled:opacity-30"
                  onClick={(e) => {
                    e.stopPropagation();
                    setCpExpIndex((cpExpIndex + 1) % cpExpFiles.length);
                  }}
                  disabled={cpExpFiles.length <= 1}
                >
                  ›
              ...
              {cpExpFiles[cpExpIndex].type === "image" ? (
                <img
                  src={cpExpFiles[cpExpIndex].src}
                  alt={cpExpFiles[cpExpIndex].label + " Certificate"}
                  className="rounded-lg shadow-md max-h-64 w-auto"
                />
              ) : (
                <iframe
                  src={cpExpFiles[cpExpIndex].src}
                  title={cpExpFiles[cpExpIndex].label + " Certificate"}
                  className="rounded-lg shadow-md w-full h-64"
                />
              )}
            </div>
      ...
      {showGssocModal && (
        <>
          <div
            className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm"
            onClick={() => setShowGssocModal(false)}
          />
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-4 w-[340px] sm:w-[400px] flex flex-col items-center border border-gray-200 dark:border-slate-700">
              <button
              ...
              <img
                src="./Richa_Kumari_Jaishwal_Badge_Contributor_GSSoC2024-Extd.png"
                alt="Richa Kumari Jaishwal GSSoC Badge"
                className="mb-2 rounded-lg shadow-md max-h-40 w-auto"
              />
              <img
                src="./gssoc_badge.jpg"
                alt="GSSoC Badge"
                className="rounded-lg shadow-md max-h-32 w-auto"
              />
            </div>
          </div>
        </>
      ...
      {showHacktoberModal && (
        <>
          <div
            className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm"
            onClick={() => setShowHacktoberModal(false)}
          />
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-4 w-[340px] sm:w-[400px] flex flex-col items-center border border-gray-200 dark:border-slate-700">
              <button
